...
import { Section, ButtonStyle, ArrowDirection } from "../models";
...
export class ArchitectureSection extends LitElement {
    ...
    override render() {
        return html`
            <div class="architecture-section architecture-section-${this.sectionStyle || 'default'}">
                ${this.arrowTemplate()}
        ...
        return  html`
            <div class="architecture-groups">
                ${this.section.groups?.map((group) =>
                    html`<architecture-group .group=${group} .buttonStyle=${this.buttonStyle}></architecture-group>`
                )}
            </div>
        `;
    }
    ...
    buttonsTemplate() {
        ...
        return this.section.buttons?.map((button) => 
            html`<architecture-button .button=${button} .buttonStyle=${this.buttonStyle}></architecture-button>`
        );
    }
        ...
        return [
            ...
            css`
                ...
                .architecture-section-side {
                    /* What if there are is no side section? How to make more dynamic? */
